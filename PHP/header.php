...
<body <?php body_class();?>>
	...
	<?php get_template_part(photolaboratory_get_file_slug('sidebar_outer.php')); ?>

	<?php
		$body_style  = photolaboratory_get_custom_option('body_style');
		$class = $style = '';
		if (photolaboratory_get_custom_option('bg_custom')=='yes' && ($body_style=='boxed' || photolaboratory_get_custom_option('bg_image_load')=='always')) {
			if (($img = photolaboratory_get_custom_option('bg_image_custom')) != '')
				$style = 'background: url('.esc_url($img).') ' . str_replace('_', ' ', photolaboratory_get_custom_option('bg_image_custom_position')) . ' no-repeat fixed;';
			else if (($img = photolaboratory_get_custom_option('bg_pattern_custom')) != '')
				$style = 'background: url('.esc_url($img).') 0 0 repeat fixed;';
			else if (($img = photolaboratory_get_custom_option('bg_image')) > 0)
				$class = 'bg_image_'.($img);
			else if (($img = photolaboratory_get_custom_option('bg_pattern')) > 0)
				$class = 'bg_pattern_'.($img);
			if (($img = photolaboratory_get_custom_option('bg_color')) != '')
	...
	<div class="body_wrap<?php echo !empty($class) ? ' '.esc_attr($class) : ''; ?>"<?php echo !empty($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
		...
		<?php
		$video_bg_show = photolaboratory_get_custom_option('show_video_bg')=='yes';
		$youtube = photolaboratory_get_custom_option('video_bg_youtube_code');
		$video   = photolaboratory_get_custom_option('video_bg_url');
		$overlay = photolaboratory_get_custom_option('video_bg_overlay')=='yes';
		if ($video_bg_show && (!empty($youtube) || !empty($video))) {
			if (!empty($youtube)) {
				...
				$ext = photolaboratory_get_file_ext($video);
		...
		<div class="page_wrap">
			...
			<?php
			$top_panel_style = photolaboratory_get_custom_option('top_panel_style');
			$top_panel_position = photolaboratory_get_custom_option('top_panel_position');
			$top_panel_scheme = photolaboratory_get_custom_option('top_panel_scheme');
			// Top panel 'Above' or 'Over'
			if (in_array($top_panel_position, array('above', 'over'))) {
				photolaboratory_show_post_layout(array(
					'layout' => $top_panel_style,
					'position' => $top_panel_position,
					'scheme' => $top_panel_scheme
					), false);
				// Mobile Menu
				get_template_part(photolaboratory_get_file_slug('templates/headers/_parts/header-mobile.php'));
			}

			// Slider
			get_template_part(photolaboratory_get_file_slug('templates/headers/_parts/slider.php'));
			...
			$show_title = photolaboratory_get_custom_option('show_page_title')=='yes';
			$show_navi = apply_filters('photolaboratory_filter_show_post_navi', false);
			$show_breadcrumbs = photolaboratory_get_custom_option('show_breadcrumbs')=='yes';
						...
						<div class="content_wrap">
							...
							if ($show_title) {
								if ($show_navi) {
									...
									?><h1 class="page_title"><?php echo strip_tags(photolaboratory_get_blog_title()); ?></h1><?php
								}
							}
							if ($show_breadcrumbs) {
								?><div class="breadcrumbs"><?php if (!is_404()) photolaboratory_show_breadcrumbs(); ?></div><?php
							}
							?>
						</div>
					</div>
			...
			<div class="page_content_wrap page_paddings_<?php echo esc_attr(photolaboratory_get_custom_option('body_paddings')); ?>">

				<?php
				// Content and sidebar wrapper
				if ($body_style!='fullscreen') photolaboratory_open_wrapper('<div class="content_wrap">');
				
				// Main content wrapper
				photolaboratory_open_wrapper('<div class="content">');
				?>